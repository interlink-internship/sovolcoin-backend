...
class Solve{
    ...
    void Calc(){
        ...
        while(true){
            ...
            if(type == 0){
                string S = re.s();
                data.Add(db.AddData(S));
                Console.WriteLine(data.Count+" "+S);
            }
            else if(type == 1){
                Console.WriteLine(db.Right() ? "OK" : "Wrong");
            }
            else if(type == 2){
                db.WriteAllData();
            }
            else if(type == 3){
                data[re.i()-1].WriteData();
            }
            else if(type == 4){
                int x = re.i()-1;
                string S = re.s();
                Console.Write(data[x].S);
                data[x].S = S; 
                Console.WriteLine(" -> "+data[x].S);
            }
            else if(type == 5){
                data[re.i()-1].ReCalc(sha);
...
class Database{
    ...
    SHA256 hash;
    public Database(SHA256 sha){
        hash = sha;
    }
    public Data AddData(string S){
        if(FirstData == null){
            FirstData = new Data(S,hash);
            LastData = FirstData;
        }
        else{
            LastData = new Data(S,LastData,hash);
        }
        return LastData;
    }
    public bool Right(){
        for(int i=0;i<8;i++){
            if(FirstData.prev_hash[i] != 0){
                return false;
            }
        }
        return FirstData.Right(hash);
    }
    public void WriteAllData(){
        FirstData.WriteAllData();
    }
}
class Data{
    public uint[] hash;
    public uint[] prev_hash;
    public string Nonce;
    public string S;
    Data Next;
    Data Before;
    public Data(string S0,SHA256 ha){
        prev_hash = new uint[8];
        S = S0;
        Random rand = new Random();
        string temp = S + ha.uintToString(prev_hash); 
        while(true){
            ...
            uint[] tempshash = ha.Hash(temps);
            if(tempshash[0]%(1 << 20) == 0){
                hash = tempshash;
                Nonce = temps.Substring(temps.Length-8,8);
                break;
            }
        }
    }
    public Data(string S0,Data prev,SHA256 ha){
        Before = prev;
        prev.Next = this;
        prev_hash = prev.hash;
        S = S0;
    ...
    public bool Right(SHA256 ha){
        uint[] has = ha.Hash(S + ha.uintToString(prev_hash) + Nonce);
        for(int i=0;i<8;i++){
            if(has[i] != hash[i]){
                return false;
            }
        ...
        if(Next != null){
            for(int i=0;i<8;i++){
                if(Next.prev_hash[i] != hash[i]){
                    return false;
                }
            }
            return Next.Right(ha);
        }
        return true; 
    }
    public void WriteAllData(){
...
class SHA256{
    ...
    public string uintToString(uint[] U){
        StringBuilder sb = new StringBuilder();
        for(int i=0;i<U.Length;i++){
            sb.Append((char)(U[i] / (1 << 24)));
            sb.Append((char)(U[i] / (1 << 16) % (1 << 8)));
            sb.Append((char)(U[i] / (1 << 18) % (1 << 8)));
            sb.Append((char)(U[i] % (1 << 8)));
        }
        return sb.ToString();
    }
